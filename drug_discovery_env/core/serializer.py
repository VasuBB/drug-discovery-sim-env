"""Compact text rendering of state for the LLM prompt."""
...
def summarize_state(state: GameState) -> str:
    ...
    if best:
        best_text = (
            f"{(best.id or best.smiles[:24])} potency={best.potency:.2f} "
            f"safety={best.safety:.2f} selectivity={best.selectivity:.2f}"
        )
    return (
        f"Disease={state.disease}; stage={state.stage}; step={state.step}/{state.max_steps}; "
