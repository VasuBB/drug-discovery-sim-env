"""Budget manager with variable-cost multipliers.

Base cost per tool comes from `config.tools.costs`. Final cost is multiplied
by signal-quality factors (low-info, high-uncertainty, late-stage redundancy)
so that spamming cheap tools at low information yield is naturally penalized.
"""
...
class BudgetManager:
    ...
    def base_cost(self, tool_name: str) -> float:
        return float(self.settings.tools.costs.get(tool_name, 0.0))
